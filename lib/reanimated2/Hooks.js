...
function isAnimated(prop) {
    ...
    return (prop === null || prop === void 0 ? void 0 : prop.onFrame) !== undefined;
}
...
export function useAnimatedStyle(updater, dependencies, adapters) {
    ...
    useEffect(() => {
        ...
        if (isJest()) {
            fun = () => {
                'worklet';
                jestStyleUpdater(sharableViewDescriptors, updater, remoteState, maybeViewRef, animationsActive, animatedStyle, adapters);
            };
        }
        else {
            fun = () => {
        ...
        const mapperId = startMapper(fun, inputs, [], upadterFn, sharableViewDescriptors);
        return () => {
            stopMapper(mapperId);
        };
    }, dependencies);
